...
const initialCategories: Record<CategoryId, StageCategory> = {
  "Property Acquisition": {
    ...
    objective: "Build your real estate portfolio by purchasing at least one property with good ROI potential",
    minCompletions: 1,
    unlocked: true,
    completed: false
  },
  "Business Ventures": {
    ...
    objective: "Diversify income through small business investments and profit-sharing agreements",
    minCompletions: 1,
    unlocked: false,
    completed: false
  },
  ...
  "Asset Financing": {
    id: "Asset Financing",
    objective: "Make an informed decision about vehicle financing and ownership",
    minCompletions: 1,
    unlocked: false,
    completed: false
  },
...
const determineObjectives = (categories: Record<CategoryId, StageCategory>): CategoryId[] => {
  if (categories["Asset Financing"].completed && categories["Market Trading"].completed) {
    return ["Recreation"]
  }
  ...
  if (!categories["Property Acquisition"].completed) {
    ...
    if (!categories["Market Trading"].completed) objectives.push("Market Trading")
    if (!categories["Asset Financing"].completed) objectives.push("Asset Financing")
  }
...
//   persist(
    (set) => ({
      ...
      completeStage: (stageId) => set((state) => {
        ...
        const completedCount = categoryStages.filter(stage => stage.completed).length
        ...
        const updatedCategories = unlockNextCategories(newCategories, categoryId)
      ...
      unlockCategory: (categoryId: CategoryId) => set((state) => {
